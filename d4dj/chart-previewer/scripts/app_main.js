...
var D4DJChartRenderer = /** @class */ (function () {
    function D4DJChartRenderer(canvas) {
    ...
    D4DJChartRenderer.prototype.fillWithScaleAndOffset = function (x, y, width, height) {
    ...
    D4DJChartRenderer.prototype.fillByTime = function (x, time, width, height, color) {
        this.ctx.fillStyle = color;
        this.fillWithScaleAndOffset(x, this.heightPerSec * (this.totalDur - (time - this.startTime)) -
            height +
            this.offsetHeight, width, height);
    };
    D4DJChartRenderer.prototype.textByTime = function (x, time, height, text, color) {
        this.ctx.fillStyle = color;
        this.ctx.font = height.toString() + "px Arial";
        this.ctx.fillText(text, x, this.heightPerSec * (this.totalDur - (time - this.startTime)) +
            this.offsetHeight);
    };
    D4DJChartRenderer.prototype.drawBarLines = function (barLines) {
        var _this = this;
        barLines.forEach(function (barLine) {
            _this.fillByTime((D4DJChartRenderer.TOTAL_WIDTH / 2) *
                D4DJChartRenderer.LANE_WIDTH, barLine, D4DJChartRenderer.TOTAL_WIDTH, 1, "#eeeeee");
    ...
    D4DJChartRenderer.prototype.drawTapNote = function (note) {
        this.fillByTime(this.getLaneWidth() * (note.LaneId + 0.5), note.Time, this.getLaneWidth() * 0.8, D4DJChartRenderer.NOTE_HEIGHT, "#00cccc");
    };
    D4DJChartRenderer.prototype.drawScratchNote = function (note) {
        this.fillByTime(this.getLaneWidth() * (note.LaneId + 0.5), note.Time, this.getLaneWidth() * 0.8, D4DJChartRenderer.NOTE_HEIGHT, "#ffcc00");
    };
    D4DJChartRenderer.prototype.drawHoldNote = function (note) {
        this.fillByTime(this.getLaneWidth() * (note.LaneId + 0.5), note.Time, this.getLaneWidth() * 0.8, D4DJChartRenderer.NOTE_HEIGHT, "#eeee00");
    };
    D4DJChartRenderer.prototype.drawStopNote = function (note) {
        this.fillByTime(this.getLaneWidth() * (note.LaneId + 0.5), note.Time, this.getLaneWidth() * 0.8, D4DJChartRenderer.NOTE_HEIGHT, "#ee0000");
    };
    D4DJChartRenderer.prototype.drawLaserNodes = function (note) {
        this.fillByTime(this.getLaneWidth() * (note.LaneId + 0.5), note.Time, this.getLaneWidth() * 0.5, D4DJChartRenderer.NOTE_HEIGHT * 2, "rgb(245, 93, 159)");
    };
    D4DJChartRenderer.prototype.drawLine = function (start, end, width, color, heightOffset) {
        var sx = this.getLaneWidth() * (start.LaneId + 0.5) +
            D4DJChartRenderer.X_OFFSET;
        var ex = this.getLaneWidth() * (end.LaneId + 0.5) +
            D4DJChartRenderer.X_OFFSET;
        var sy = this.heightPerSec *
            (this.totalDur - (start.Time - this.startTime)) +
            this.offsetHeight -
            heightOffset;
        var ey = this.heightPerSec * (this.totalDur - (end.Time - this.startTime)) +
            this.offsetHeight;
        this.ctx.beginPath();
    ...
    D4DJChartRenderer.prototype.drawFlickArrow = function (note) {
        var x = this.getLaneWidth() * (note.LaneId + 0.5) +
            D4DJChartRenderer.X_OFFSET;
        var y = this.heightPerSec * (this.totalDur - (note.Time - this.startTime)) +
            this.offsetHeight -
            D4DJChartRenderer.NOTE_HEIGHT;
        var x1 = x + (Math.abs(note.Direction) - 1.5) * (note.Direction > 0 ? 1 : -1) * this.getLaneWidth();
        var x2 = x + (Math.abs(note.Direction) - 1) * (note.Direction > 0 ? 1 : -1) * this.getLaneWidth();
    ...
    D4DJChartRenderer.prototype.renderChart = function (chart) {
        ...
        lasers.forEach(function (pair) {
            _this.drawLine(pair[0], pair[1], _this.getLaneWidth() * 0.1, "rgba(245, 93, 159, 0.8)", D4DJChartRenderer.NOTE_HEIGHT * 2);
        });
        // Draw holds
        longs.forEach(function (pair) {
            _this.drawLine(pair[0], pair[1], _this.getLaneWidth() * 0.5, "rgba(220, 220, 0, 0.8)", D4DJChartRenderer.NOTE_HEIGHT);
        });
        // Draw stop
        stops.forEach(function (pair) {
            _this.drawLine(pair[0], pair[1], _this.getLaneWidth() * 0.5, "rgba(220, 0, 0, 0.8)", D4DJChartRenderer.NOTE_HEIGHT);
        });
        // Draw notes
        ...
        for (var i = 0; i < clips; i++) {
            this.targetCtx.drawImage(this.canvas, 0, this.height - (i + 1) * this.targetCanvas.height, D4DJChartRenderer.TOTAL_WIDTH, this.targetCanvas.height, i * D4DJChartRenderer.TOTAL_WIDTH, 0, D4DJChartRenderer.TOTAL_WIDTH, this.targetCanvas.height);
        }
    };
    D4DJChartRenderer.TOTAL_WIDTH = 200;
    D4DJChartRenderer.LANE_WIDTH = 0.7;
    D4DJChartRenderer.X_OFFSET = (D4DJChartRenderer.TOTAL_WIDTH -
        D4DJChartRenderer.TOTAL_WIDTH * D4DJChartRenderer.LANE_WIDTH) /
        2;
    D4DJChartRenderer.NOTE_HEIGHT = 5;
    return D4DJChartRenderer;
}());
var cvs = document.querySelector("canvas");
var input = document.querySelector("#input");
var renderer = new D4DJChartRenderer(cvs);
var chartData = null;
input.addEventListener("change", function () {
    var file = input.files[0];
    var reader = new FileReader();
    reader.onload = function () {
        renderer.renderChart(JSON.parse(reader.result));
    };
